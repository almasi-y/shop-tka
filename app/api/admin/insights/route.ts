...
export async function GET(request: Request) {
  try {
    ...
    try {
      await client.withConfig({ token, useCdn: false }).users.getById("me");
    } catch {
      return Response.json({ error: "Invalid Sanity session" }, { status: 401 });
    }
    ...
    const normalizedProductSales = productSales.flatMap((order) =>
      order.products?.length
        ? order.products.map((product, index) => ({
            productId: product.productId,
            productName: product.productName,
            productPrice:
              order.productPrices?.[index] ?? product.productPrice,
            quantity: order.quantities?.[index] ?? 0,
          }))
        : (order.legacyItems ?? []).map((item) => ({
    ...
    for (const sale of normalizedProductSales) {
    ...
    const slowMoving = productsInventory
      .filter((p) => {
        const salesQty = productSalesById.get(p._id) || 0;
        return (p.stock ?? 0) > 10 && salesQty === 0;
      })
      .slice(0, 5);

    // Helper to calculate days since order
    const getDaysSinceOrder = (createdAt: string | null) => {
    ...
    const dataSummary = {
      ...
      inventory: {
        ...
        lowStockCount: productsInventory.filter((p) => (p.stock ?? 0) <= 5)
          .length,
      },
      operations: {
        statusDistribution,
...
Generate insights in the required JSON format.`,
    ...
    try {
      ...
      insights = {
        salesTrends: {
          summary: `Revenue this week: KSh ${currentRevenue.toFixed(2)} (${revenueChange > 0 ? "+" : ""}${revenueChange.toFixed(1)}% vs last week)`,
          highlights: [
            `${revenuePeriod.currentOrderCount || 0} orders this week`,
            `Average order value: KSh ${avgOrderValue.toFixed(2)}`,
            topProducts[0]
